(defpackage #:systems-indexer/core
  (:use #:cl)
  (:import-from #:dexador)
  ...
  (:import-from #:rutils
                #:fmt))
(in-package systems-indexer/core)

;; http://nocf-www.elastic.co/guide/en/elasticsearch/reference/current/getting-started-search.html


(defun index (collection id data)
  (let ((content (jonathan:to-json data))
        (url (fmt "http://localhost:9200/~A/_doc/~A"
                  collection
                  (quri:url-encode id))))
    (jonathan:parse
     (dex:put url
...
(defun search-objects (collection term)
  (loop with url = (fmt "http://localhost:9200/~A/_search"
                        collection)
        with query = (list
                      :|query|
                      (list :|match_phrase|
            ...
            (symbol
             (concatenate
              ...
              (if (multiple-value-bind
                  ...
                  ":" "::")
              (symbol-name symbol)))
            ;; TODO: придумать что-то с typespecciier которые не являются
            ;;       символми а являются объектами
            (t symbol))))
  ...
  (let ((current-package (etypecase package
    ...
    (do-external-symbols (symbol current-package)
      (let ((full-symbol-name (encode-symbol symbol))
            ;; (is-external (is-external symbol package))
            )
